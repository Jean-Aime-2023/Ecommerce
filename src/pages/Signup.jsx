  ...
  return (
    <div className='poppins flex flex-col justify-center items-center gap-8 2xl:w-[35%] lg:w-[50%] max-lg:w-full max-lg:container max-md:px-12 max-lg:px-[6rem] px-20 py-10 2xl:ml-[10rem] relative'>
      ...
      <form action="" className='flex flex-col w-full gap-5'>
        ...
        <section className='flex flex-col w-full gap-3'>
          <label htmlFor="password"  className='text-lg max-md:text-base'>Password <span className='text-[#DB4444]'>*</span></label>
          <input type="password" name='password' required className='px-4 py-3 rounded-md border border-gray-500 bg-transparent text-black outline-none focus:outline-none' />
        </section>
        <section className='flex flow-row items-center gap-2'>
          <Checkbox />
          <p className='text-lg max-md:text-base'>I accept all <span className='text-[#DB4444]'>Terms</span> and <span className='text-[#DB4444]'>Conditions</span></p>
        </section>
        <button type='submit' className="bg-[#DB4444] text-white py-4 text-lg font-semibold poppins leading-[24px] rounded-md">START SHOPPING</button>
      </form>
      <Link to={'/signin'} className='text-lg text-[#DB4444] max-md:text-base'>Already have an account?</Link>
